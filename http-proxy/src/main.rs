...
use hyper_trust_dns::{TrustDnsHttpConnector, new_trust_dns_http_connector};
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let client: Client<_, Body> = Client::builder().build(new_trust_dns_http_connector());
...
async fn handle_request(
    client: Client<TrustDnsHttpConnector, Body>,
    ...
    request
        .headers_mut()
        .insert(HOST, HeaderValue::from_static("127.0.0.1:65535"));
    ...
    let mut uri_string = format!("http://127.0.0.1:65535{}{}", api_path, trimmed_path);
